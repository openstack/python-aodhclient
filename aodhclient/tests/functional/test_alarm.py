...
class AodhClientPrometheusRulesTest(base.ClientTestBase):
    ...
    def test_prometheus_type_scenario(self):
        ...
        req = requests.get(
            os.environ.get("PROMETHEUS_ENDPOINT") +
        ...
        # GET BY NAME
        result = self.aodh(
            'alarm', params="show --name alarm_p1")
        alarm_show = self.details_multiple(result)[0]
        self.assertEqual(ALARM_ID, alarm_show["alarm_id"])
        self.assertEqual('alarm_p1', alarm_show['name'])
        self.assertEqual('90.0', alarm_show['threshold'])
        ...
        self.assertEqual(sorted_names, names)
        self.assertEqual(sorted_ids, ids_with_same_name)
        # List with sort with key=name dir=desc and with the marker
        # verify that we handle non full and empty pages correctly.
        # Verify that marker works as expected

        # We have 3 alarms. List the first 2.
        result = self.aodh(
            'alarm',
            params="list --filter all_projects=true --filter "
                   "type=prometheus --sort name:asc --limit 2")
        alarm_list = self.parser.listing(result)
        self.assertNotIn('alarm_ptc',
                         [r['name'] for r in alarm_list])
        self.assertEqual(2, len(alarm_list))

        # List the next page. Only the 3rd alarm should be returned
        result = self.aodh(
            'alarm',
            params="list --filter all_projects=true --filter "
                   "type=prometheus --sort name:asc "
                   "--marker %s --limit 2" % alarm_list[1]['alarm_id'])
        alarm_list = self.parser.listing(result)
        self.assertIn('alarm_ptc',
                      [r['name'] for r in alarm_list])
        self.assertEqual(1, len(alarm_list))

        # List the next page. Empty page should be returned
        result = self.aodh(
            'alarm',
            params="list --filter all_projects=true --filter "
                   "type=prometheus --sort name:asc "
                   "--marker %s --limit 2" % alarm_list[0]['alarm_id'])
        alarm_list = self.parser.listing(result)
        self.assertEqual(0, len(alarm_list))

        # Delete the last created alarm
        self.aodh('alarm', params="delete %s" % created_alarm_id)
        # Delete alarm_ptc
